...
#include "menu_item/menu_item_bright_lantern.h"
#include "menu_item/menu_item_cinematic_test.h"
#include "menu_item/menu_item_debug_screen.h"
#include "menu_item/menu_item_false_accusations.h"
#include "menu_item/menu_item_female_horse.h"
#include "menu_item/menu_item_hat_tracker.h"
#include "menu_item/menu_item_headshot_tracker.h"
#include "menu_item/menu_item_max_horse_stats.h"
#include "menu_item/menu_item_night_light.h"
#include "menu_item/menu_item_rainbow_lantern.h"
#include "menu_item/menu_item_refill_ammo.h"
#include "menu_item/menu_item_refill_inventory.h"
...
void add_teleport_menu_item(MenuBase* menu, const std::string& place, const Vector3& coordinates) {	
	menu->AddItem(new MenuItemPlayerTeleport(place, coordinates));
}
...
MenuBase *CreatePlayerTeleportMenu(MenuController *controller)
{
	...
	map<string, Vector3> locations_map;
	locations_map["South Map"] = {-5311.2583f, -4612.00f, -10.63389f};
	locations_map["Guarma"] = {1315.66381f, -6815.48f, 42.377101f};
	locations_map["Annesburg"] = {2898.593994f, 1239.85253f, 44.073299f};
	locations_map["Strawberry"] = {-1725.22143f, -418.11560f, 153.55740f};
	locations_map["Valentine"] = {-213.152496f, 691.802979f, 112.37100f};
	locations_map["Rhodes"] = {1282.707520f, -1275.7485f, 74.945099f};
	locations_map["Saint Denis"] = {2336.584961f, -1106.2358f, 44.737598f};
	locations_map["Wapiti"] = {538.738525f, 2217.46557f, 240.23280f};
	locations_map["Butcher Creek"] = {2552.203613f, 835.510010f, 81.183098f};
	locations_map["Blackwater"] = {-798.338379f, -1238.9395f, 43.537899f};
	locations_map["Beechers"] = {-1653.19738f, -1448.8156f, 82.503502f};
	locations_map["Caliga Hall"] = {1705.509888f, -1386.3237f, 42.884998f};
	locations_map["Braithwaite"] = {1011.190674f, -1661.6768f, 45.918301f};	
	locations_map["Van Horn"] = {2982.234863f, 445.724915f, 51.491501f};
	locations_map["Cornwall"] = {437.7247920f, 494.582092f, 107.67649f};
	locations_map["Colter"] = {-1371.6590f, 2388.5073f, 307.7218f};
	locations_map["Emerald Ranch"] = {1332.332642f, 300.425110f, 86.306297f};
	locations_map["Pronghorn"] = {-2616.57714f, 519.256775f, 144.10809f};
	locations_map["Manzanita Post"] = {-1977.98754f, -1545.6749f, 112.87020f};
	locations_map["Lagaras"] = {2111.099121f, -662.25317f, 41.259899f};
	locations_map["Armadillo"] = {-3622.65527f, -2586.5795f, -15.36900f};
	locations_map["Tumbleweed"] = {-5382.39453f, -2940.1596f, 1.582700f};	
	locations_map["MacFarlanes"] = {-2296.26318f, -2454.4101f, 60.969898f};
	locations_map["Benedict Point"] = {-5269.60400f, -3411.0588f, -23.15930f};

	for (const auto& location : locations_map) {
		add_teleport_menu_item(menu, location.first, location.second);
	}
...
MenuBase* create_jtmcraft_menu(MenuController* controller) {
	const auto menu = new MenuBase(new MenuItemTitle("Jtmcraft"));
	controller->RegisterMenu(menu);

	std::map<std::string, MenuItemBase*> item_map;
	if (configuration::get_instance().get_or_else("jtmcraft:menu_items.hat_tracker", false)) {
		item_map["Hat tracker"] = new menu_item_hat_tracker("Hat tracker");
	}
	if (configuration::get_instance().get_or_else("jtmcraft:menu_items.headshot_tracker", false)) {
		item_map["Headshot tracking"] = new menu_item_headshot_tracker("Headshot tracking");
	}
	if (configuration::get_instance().get_or_else("jtmcraft:menu_items.night_light", false)) {
		item_map["Night light"] = new menu_item_night_light("Night light");
	}
	if (configuration::get_instance().get_or_else("jtmcraft:menu_items.bright_lantern", false)) {
		item_map["Bright lantern"] = new menu_item_bright_lantern("Bright lantern");
	}
	if (configuration::get_instance().get_or_else("jtmcraft:menu_items.rainbow_lantern", false)) {
		item_map["Rainbow lantern"] = new menu_item_rainbow_lantern("Rainbow lantern");
	}
	if (configuration::get_instance().get_or_else("jtmcraft:menu_items.debug_screen", false)) {
		item_map["Show position"] = new menu_item_debug_screen("Show position");
	}
	if (configuration::get_instance().get_or_else("jtmcraft:menu_items.refill_inventory", false)) {
		item_map["Refill inventory"] = new menu_item_refill_inventory("Refill inventory");
	}
	if (configuration::get_instance().get_or_else("jtmcraft:menu_items.refill_ammo", false)) {
		item_map["Refill ammo"] = new menu_item_refill_ammo("Refill ammo");
	}
	if (configuration::get_instance().get_or_else("jtmcraft:menu_items.false_accusations", false)) {
		item_map["Enable false accusations"] = new menu_item_false_accusations("Enable false accusations");
	}
	if (configuration::get_instance().get_or_else("jtmcraft:menu_items.female_horse", false)) {
		item_map["Female horse"] = new menu_item_female_horse("Female horse");
	}
	if (configuration::get_instance().get_or_else("jtmcraft:menu_items.max_horse_stats", false)) {
		item_map["Max horse stats"] = new menu_item_max_horse_stats("Max horse stats");
	}
	if (configuration::get_instance().get_or_else("jtmcraft:menu_items.cinematic_test", false)) {
		menu->AddItem(new menu_item_cinematic_test("Cinematic test"));
	}
	
	for (const auto& pair : item_map) {
		menu->AddItem(pair.second);
	}
...
MenuBase *CreateMainMenu(MenuController *controller)
{
	...
	menu->AddItem(new MenuItemMenu("Jtmcraft", create_jtmcraft_menu(controller)));
	
	return menu;
}

// void play_music(const char* musicName) {
// 	graphics_util::wait_and_draw_center_text(10000, musicName, ui_color::red_opaque);
// 	AUDIO::PREPARE_MUSIC_EVENT(const_cast<char*>(musicName));
// 	AUDIO::TRIGGER_MUSIC_EVENT(const_cast<char*>(musicName));
// 	WAIT(10000);
// 	AUDIO::TRIGGER_MUSIC_EVENT(const_cast<char*>(musicName));
//
// 	WAIT(500);
...
#define JTMCRAFT_JSON "jtmcraft.json"
...
void main()
{
	...
	{		
		...
		if (IsKeyJustUp(0x55)) {
			// U key; TODO: add a menu item for this
			configuration::get_instance().load(JTMCRAFT_JSON);
		}
		const bool infinite_ammo = configuration::get_instance().get_or_else("jtmcraft:cheats.ammo.infinite", false);
		WEAPON::_0xFBAA1E06B6BCA741(player_ped, infinite_ammo);
...
void create_console() {
	if (configuration::get_instance().get_or_else("jtmcraft:console", false)) {
		if(AllocConsole()) {
			FILE* fp;
			...
			std::wcout << L"Console created successfully" << '\n';
		}
	}
}
